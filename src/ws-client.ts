...
import { debounceArgs, isSecurePage, randomString, stringifyWithPreservingUndefinedAsNull } from './utils';
...
class Api {
    ...
    urlProvider = async () => {
        ...
        let token = new URLSearchParams(window.location.search).get('token')
            ?? local.get(TOKEN_LOCAL_STORAGE_ITEM_NAME);
        ...
        switch (data.topic) {
            ...
            case 'bridge/response/networkmap': {
                ...
                if (response.status === 'ok') {
                    stateUpdate['networkGraph'] = (response.data as {
                        value: GraphI
                    }).value;
                }
