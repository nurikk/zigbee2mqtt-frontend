    ...
    render() {
        ...
        if (groups && groups.length) {
            const groupOptions = groups.map((group) => (
                <option key={group.id} value={`group${DELIMITER}${group.friendly_name}`}>
                    {group.friendly_name}
                </option>
            ));
            options.push(
                <optgroup key="Groups" label="Groups">
                    {groupOptions}
                </optgroup>,
            );
            options.push(
        ...
        return (
            <select defaultValue={`${type}${DELIMITER}${value}`} onChange={this.onSelect} className="form-control">
                {options}
            </select>
        );
    }
