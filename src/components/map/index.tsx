...
export class Map extends Component<GlobalState & MapApi, MapState> {
    ...
    updateNodes = (): void => {
        this.updateForces();
        const node = d3.selectAll<SVGGeometryElement, NodeI>(`.${style.node}`);
        const link = d3.selectAll<SVGLineElement, LinkI>(`.${style.link}`);
        const linkLabel = d3.selectAll<SVGLineElement, LinkI>(`.${style.linkLabel}`);
        const ticked = (): void => {
            const radius = 40;
            ...
            node.attr(
                'transform',
                (d) =>
                    `translate(${Math.max(radius, Math.min(width - radius, d.x)) - imgXShift}, ${
                        Math.max(radius, Math.min(height - radius, d.y)) - imgYShift
                    })`,
            );
        };
        const { networkGraph } = this.props;
        this.simulation.nodes(networkGraph.nodes).on('tick', ticked);
        const linkForce = this.simulation.force('link') as d3.ForceLink<NodeI, LinkI>;
    ...
    updateForces(): void {
        ...
        const linkForce = d3
            ...
            .strength(0.2);
        const chargeForce = d3.forceManyBody().distanceMin(200).distanceMax(1000).strength(-200);

        const repelForce = d3.forceManyBody().strength(-140).distanceMax(50).distanceMin(20);

        const collisionForce = d3.forceCollide(40).strength(1).iterations(50);
                ...
                <div className="row h-100 justify-content-center align-items-center">
                    {networkGraphIsLoading ? (
                        <div className="justify-content-center align-items-center">
                            ...
                            <div className="row">
                                <div className="col-6 mx-auto">
                                    Depending on the size of your network this can take somewhere between 10 seconds and
                                    2 minutes.
                                </div>
                            </div>
                        </div>
                        ...
                        <div className=" justify-content-center align-items-centerr">
                            <div className="row">
                                <div className="col-6 mx-auto">No map data.</div>
                            </div>
                            <div className="row">
                                <div className="col-6 mx-auto">
