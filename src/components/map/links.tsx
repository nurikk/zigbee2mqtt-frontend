...
const Link: FunctionComponent<LinkProps> = (props) => {
    ...
    return <path id={id} className={cx(style.link, style[linkType])} strokeWidth={1} fill="transparent" {...rest} />;
};
...
const Links: FunctionComponent<LinksPros> = (props) => {
    ...
    useLayoutEffect(() => {
        select(ref.current as SVGGElement)
            .selectAll(`.${style.link}`)
            .data(links);
        select(ref.current as SVGGElement)
            .selectAll(`.${style.linkLabel}`)
            .data(links);
    }, [links.length]);
            ...
            {links.map((link: LinkI) => {
                ...
                return (
                    <Fragment key={`fragment${id}`}>
                        <Link id={`edgepath${id}`} link={link} />
                        <LinkLabel xlinkHref={`#edgepath${id}`} link={link} />
                    </Fragment>
                );
            })}
        </g>
