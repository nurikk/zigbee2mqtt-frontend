...
const getStarShape = (innerCircleArms: number, styleStarWidth: number, innerOuterRadiusRatio: number): string => {
    return calcStarPoints(15, 15, innerCircleArms, styleStarWidth, innerOuterRadiusRatio);
};
        ...
        return (
            ...
            >
                {node.type === 'Coordinator' ? (
                    ...
                    <image width={32} height={32} onError={onImageError} className={`${style.img}`} href={imgUrl} />
                )}
...
export default class Nodes extends Component<NodesProps, NodesState> {
    updateDrag(): void {
        ...
        d3.selectAll<SVGCircleElement, NodeI>(`.${style.node}`).call(dragForce);
    }
