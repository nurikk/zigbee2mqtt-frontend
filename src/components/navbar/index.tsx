...
const StartStopJoinButton: FunctionComponent<
    StartStopJoinButtonProps & Pick<BridgeApi, 'setPermitJoin'> & Pick<GlobalState, 'bridgeInfo'>
> = ({ devices, setPermitJoin, bridgeInfo }) => {
    ...
    return (
        <div className="input-group w-auto">
            <button onClick={onBtnClick} type="button" className="btn btn-outline-secondary">
                {bridgeInfo.permit_join ? 'Disable join' : 'Permit join'} ({selectedRouter?.friendly_name ?? 'All'})
            </button>
            {routers.length ? (
                <>
                    <Button<boolean>
                    ...
                    <ul
                        ref={ref}
                        className={cx('dropdown-menu', style['scrollable-menu'], { show: isComponentVisible })}
                    >
                        <li key="all">
                            <Button<Device> item={null} className="dropdown-item" onClick={selectAndHide}>
                                All
                            </Button>
                        </li>
                        {routers}
...
const NavBar: FunctionComponent<PropsFromStore & BridgeApi & Pick<GlobalState, 'bridgeInfo'>> = ({
    devices,
    setPermitJoin,
    bridgeInfo,
}) => {
    ...
    return (
        <nav className="navbar navbar-expand-md navbar-dark bg-dark fixed-top">
            <div ref={ref} className="container-fluid">
                <Link onClick={() => setnavbarIsVisible(false)} className="navbar-brand" to="/">
                    Zigbee2MQTT
                </Link>
                <button
                    onClick={() => {
