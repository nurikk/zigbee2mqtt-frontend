...
export function StartStopJoinButton({ devices, setPermitJoin, bridgeInfo }: StartStopJoinButtonProps) {
    ...
    const prioritizeCoordinator = (a: Device, b: Device) => a.type === 'Coordinator' ? -1 : b.type === 'Coordinator' ? 1 : 0;
    ...
    return <div className="btn-group text-nowrap me-1">
        <button onClick={onBtnClick}
            type="button"
            className="btn btn-outline-secondary">{buttonLabel}
        </button>
        {routers.length ? (<>
            <Button<boolean> type="button" className="btn btn-outline-secondary dropdown-toggle dropdown-toggle-split"
