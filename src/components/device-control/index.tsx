...
export class DeviceControlGroup extends Component<
    DeviceControlGroupProps & DeviceApi & OtaApi & GlobalState,
    DeviceControlGroupState
> {
    state = {
    ...
    render() {
        ...
        return (
            <div className="btn-group btn-group-sm" role="group">
                <Button<void> className="btn btn-secondary" onClick={this.toggleRenameModal} title="Rename device">
                ...
                <Modal isOpen={isRenameModalOpened}>
                    ...
                    <ModalBody>
                        <div className="mb-3">
                            <label htmlFor={`fn${device.ieee_address}`} className="form-label">
                                Friendly name
                            </label>
                            <input
                                id={`fn${device.ieee_address}`}
                        ...
                        {bridgeInfo.config.homeassistant ? (
                            <div className="form-check form-switch">
                                <input
                                    ...
                                    checked={renameParams.isHassRename}
                                    type="checkbox"
                                    id={`hass${device.ieee_address}`}
                                    onChange={this.onHassEntityIdChange}
                                />
                                <label className="form-check-label" htmlFor={`hass${device.ieee_address}`}>
                                    Update Home Assistant entity ID
                                </label>
                            </div>
                        ) : null}
                    </ModalBody>
                    <ModalFooter>
                        <button type="button" className="btn btn-secondary" onClick={this.toggleRenameModal}>
                            Close
                        </button>
                        <button type="button" className="btn btn-primary" onClick={this.onRenameClick}>
                            Save changes
                        </button>
                    </ModalFooter>
                </Modal>
                ...
                {state?.update?.state === 'available' ? (
                    <Button<string>
                        ...
                        promt
                    >
                        <i className={cx('fa', 'fa-cloud-download-alt')} />
                    </Button>
                ) : (
                    <Button<string>
                        className="btn btn-secondary"
                    ...
                    <ModalBody>
                        <div className="form-check form-switch">
                            <input
                                className="form-check-input"
                                name="force"
                                checked={removeParams.force}
                                type="checkbox"
                                id={`force${device.ieee_address}`}
                                onChange={this.onDeviceRemovalParamChange}
                            />
                            <label className="form-check-label" htmlFor={`force${device.ieee_address}`}>
                                Force remove
                            </label>
                        </div>
                        <div className="form-check form-switch">
                            <input
                                className="form-check-input"
                                name="block"
                                checked={removeParams.block}
                                type="checkbox"
                                id={`block${device.ieee_address}`}
                                onChange={this.onDeviceRemovalParamChange}
                            />
                            <label className="form-check-label" htmlFor={`block${device.ieee_address}`}>
                                Block from joining again
                            </label>
                        </div>
                    </ModalBody>
                    <ModalFooter>
                        <button type="button" className="btn btn-secondary" onClick={this.toggleDeviceRemovalModal}>
                            Close
                        </button>
                ...
                <button onClick={this.toggleDeviceRemovalModal} className="btn btn-danger" title="Remove device">
...
const ConnectedDeviceControlGroup = connect<DeviceControlGroupProps, {}, GlobalState, DeviceApi & OtaApi>(
    mappedProps,
    actions,
)(DeviceControlGroup);
